...
from KALVANMUSIC import YouTube, app
from KALVANMUSIC.core.call import KALVAN
from KALVANMUSIC.misc import SUDOERS, db
...
from KALVANMUSIC.utils.decorators import ActualAdminCB, languageCB
from KALVANMUSIC.utils.formatters import seconds_to_min, time_to_seconds
from KALVANMUSIC.utils.inline import close_markup
from KALVANMUSIC.utils.inline.play import colored_stream_markup, colored_stream_markup_timer, generate_progress_bar
from KALVANMUSIC.utils.colored_buttons import (
    styled_button,
...
from KALVANMUSIC.utils.thumbnails import get_thumb
...
async def manage_callback(client, callback: CallbackQuery, _):
    ...
    if command == "Pause":
        ...
        await KALVAN.resume_stream(chat_id)
        await callback.message.reply_text(
        ...
        await callback.answer()
        await KALVAN.stop_stream(chat_id)
        ...
        await KALVAN.mute_stream(chat_id)
        ...
        await KALVAN.unmute_stream(chat_id)
...
async def handle_skip_replay(callback: CallbackQuery, _, chat_id: int, command: str, user_mention: str):
        ...
        try:
            ...
            if not playlist or len(playlist) == 0:
                if await is_autoplay_on(chat_id):
                    try:
                        await KALVAN.stop_stream(chat_id)
                        from KALVANMUSIC.utils.database import remove_active_chat, remove_active_video_chat
                ...
                return await KALVAN.stop_stream(chat_id)
        except Exception:
            try:
                await callback.edit_message_text(text_msg)
                await callback.message.reply_text(
    ...
    if "live_" in queued:
        ...
        try:
            ...
            image = None
        try:
            await KALVAN.skip_stream(chat_id, new_link, video=status, image=image)
        except Exception:
        ...
        try:
            ...
            image = None
        try:
            await KALVAN.skip_stream(chat_id, file_path, video=status, image=image)
        except Exception:
            return await mystic.edit_text(_["call_6"])
        buttons = colored_stream_markup(_, chat_id, autoplay_status=ap_status)
        img = await get_thumb(videoid)
        ...
        try:
            await KALVAN.skip_stream(chat_id, videoid, video=status)
        except Exception:
            return await callback.message.reply_text(_["call_6"])
        buttons = colored_stream_markup(_, chat_id, autoplay_status=ap_status)
        ...
        if videoid in ["telegram", "soundcloud"]:
            ...
            try:
                ...
                image = None
        try:
            await KALVAN.skip_stream(chat_id, queued, video=status, image=image)
        except Exception:
            return await callback.message.reply_text(_["call_6"])
        if videoid == "telegram":
            buttons = colored_stream_markup(_, chat_id, autoplay_status=ap_status)
...
async def _now_playing_timer():
    ...
    """
    from KALVANMUSIC.utils.inline.play import UPDATE_INTERVAL, should_update_progress
    while True:
