...
import config
from KALVANMUSIC import LOGGER, app, userbot
from KALVANMUSIC.core.call import KALVAN
from KALVANMUSIC.misc import sudo
from KALVANMUSIC.plugins import ALL_MODULES
from KALVANMUSIC.utils.database import get_banned_users, get_gbanned
from KALVANMUSIC.utils.cookie_handler import fetch_and_store_cookies
...
async def init():
    ...
    try:
        ...
        LOGGER("KALVANMUSIC").info("ʏᴏᴜᴛᴜʙᴇ ᴄᴏᴏᴋɪᴇs ʟᴏᴀᴅᴇᴅ sᴜᴄᴄᴇssғᴜʟʟʏ ✅")
    except Exception as e:
        LOGGER("KALVANMUSIC").warning(f"⚠️ ᴄᴏᴏᴋɪᴇ ᴇʀʀᴏʀ: {e}")
    ...
    for all_module in ALL_MODULES:
        importlib.import_module("KALVANMUSIC.plugins" + all_module)

    LOGGER("KALVANMUSIC.plugins").info("ᴍᴏᴅᴜʟᴇs ʟᴏᴀᴅᴇᴅ...")

    await userbot.start()
    await KALVAN.start()

    try:
        await KALVAN.stream_call("https://files.catbox.moe/4c18a3.mp4")
    except NoActiveGroupCall:
        LOGGER("KALVANMUSIC").error(
    ...
    await KALVAN.decorators()
    LOGGER("KALVANMUSIC").info("✅ Kalvan music Bot Started Successfully!")
    await idle()
    await app.stop()
    await userbot.stop()
    LOGGER("KALVANMUSIC").info("sᴛᴏᴘᴘɪɴɢ ᴍᴜsɪᴄ ʙᴏᴛ ...")
            ...
            try:
                ...
                if _sync_db is not None:
                    try:
                        ...
                        LOGGER("KALVANMUSIC").warning(f"DB stats error: {db_error}")

                return f"""<!DOCTYPE html>
<html><head><title>Kalvan Music Bot</title>
...
<div class="h"><h1>🎵 Kalvan Music Bot</h1><div class="sb"><span class="dot"></span><span style="color:#00ff88;font-size:.9em">Online & Running</span></div></div>
<div class="sg">
...
<div class="f"><p>Auto-refreshes every 30s</p><p style="margin-top:8px"><a href="https://t.me/{config.BOT_USERNAME}">Open Bot</a> • <a href="https://t.me/ItsMeKalvanBots">Developer</a></p></div>
</div></body></html>"""
            except Exception as e:
                LOGGER("KALVANMUSIC").error(f"Flask home error: {e}")
        ...
        def api_stats():
            ...
            try:
                ...
                LOGGER("KALVANMUSIC").error(f"Flask API stats error: {e}")
