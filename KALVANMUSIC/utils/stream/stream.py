...
import config
from KALVANMUSIC import Carbon, YouTube, app
...
from KALVANMUSIC.utils.database import add_active_video_chat, is_active_chat
from KALVANMUSIC.utils.exceptions import AssistantErr
from KALVANMUSIC.utils.inline import aq_markup, close_markup
...
from KALVANMUSIC.utils.stream.queue import put_queue, put_queue_index
from KALVANMUSIC.utils.thumbnails import get_thumb
from KALVANMUSIC.utils.errors import capture_internal_err
...
async def stream(
    ...
    if forceplay:
        await KALVAN.force_stop_stream(chat_id)
    ...
    if streamtype == "playlist":
        ...
        if await is_active_chat(chat_id):
            ...
            if not forceplay:
                db[chat_id] = []
            await KALVAN.join_call(chat_id, original_chat_id, file_path, video=False)
            if not await is_active_chat(chat_id):
                raise AssistantErr(_["call_6"])
            await put_queue(
                chat_id,
        ...
        if await is_active_chat(chat_id):
            ...
            if not forceplay:
                db[chat_id] = []
            await KALVAN.join_call(chat_id, original_chat_id, file_path, video=is_video)
            if not await is_active_chat(chat_id):
                raise AssistantErr(_["call_6"])
            await put_queue(
                chat_id,
